...
`To-Do List` - это веб-приложение для управления задачами, разработанное на основе фреймворка `Symfony 8.0`

Приложение предоставляет полный набор инструментов для управления задачами, включая регистрацию пользователей, авторизацию, ролевую модель доступа и продвинутые функции управления задачами.
...
- assignedUser: Пользователь, которому назначена задача (ManyToOne)
- user: Пользователь, создавший задачу (ManyToOne)
...
Приложение поддерживает `SQLite` и `MySQL`

Конфигурация находится в `config/packages/doctrine.yaml`
...
Приложение использует систему миграций `Doctrine`
...
Приложение использует встроенный механизм кэширования `Symfony`

**Кэш очищается командой:**
...
Приложение отправляет email-уведомления через `Symfony Mailer`
