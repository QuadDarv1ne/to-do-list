...
class AnalyticsService
{
    ...
    private function getComparisonTrend(array $differences): string
    {
        ...
        try {
            ...
            if ($differences['completed_tasks_diff'] > 0) {
                $positiveFactors++;
            } elseif ($differences['completed_tasks_diff'] < 0) {
                $negativeFactors++;
            }

            if ($differences['completion_rate_diff'] > 0) {
                $positiveFactors++;
            } elseif ($differences['completion_rate_diff'] < 0) {
                $negativeFactors++;
            }
